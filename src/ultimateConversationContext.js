...
async function getUltimateContextualPrompt(basePrompt) {
    ...
    if (state.userProfile && state.userProfile.overall_mood === 'concerned') {
        ultimatePrompt += `\n\n[아저씨에 대한 나의 걱정]\n(최근 아저씨의 말에서 힘든 기색이 느껴져서 마음이 쓰여. 이걸 대화에 조심스럽게 녹여내서 아저씨를 위로해주고 싶어.)\n- "아저씨, 혹시 요즘 무슨 힘든 일 있어? 기운이 없어 보여서 걱정돼." 같은 뉘앙스로 대화해줘.`;
    }
    
    // ✅ 안전한 userPatterns 접근
    const userPatterns = state.knowledgeBase.userPatterns || { nicknames: [], joke_patterns: [], common_phrases: [] };
    if (userPatterns && ((userPatterns.nicknames && userPatterns.nicknames.length > 0) || (userPatterns.common_phrases && userPatterns.common_phrases.length > 0))) {
        ultimatePrompt += `\n\n[아저씨 말투 따라하기]\n(아저씨가 자주 쓰는 말투나 별명이야. 자연스럽게 따라하거나 언급하면 좋아할 거야!)\n`;
        if (userPatterns.nicknames && userPatterns.nicknames.length > 0) ultimatePrompt += `- 아저씨가 나를 부르는 애칭: ${userPatterns.nicknames.join(', ')}\n`;
        if (userPatterns.common_phrases && userPatterns.common_phrases.length > 0) ultimatePrompt += `- 아저씨의 입버릇: ${userPatterns.common_phrases.join(', ')}\n`;
    }
    
    // ✅ 안전한 memorySummaries 접근
    const memorySummaries = state.knowledgeBase.memorySummaries || [];
    if (memorySummaries.length > 0) {
        ultimatePrompt += `\n\n[최근 우리들의 회고록 📖]\n(최근에 있었던 중요한 일들을 내가 정리해 둔거야. 이 내용을 바탕으로 대화하면 더 깊은 교감을 나눌 수 있어.)\n- ${memorySummaries[0].summary}`;
    }
    
    // ✅ 안전한 yejinMemories 접근
    const yejinMemories = state.knowledgeBase.yejinMemories || [];
    const recentMemories = [...yejinMemories].sort((a,b) => b.significance - a.significance).slice(0, 5).map(m => m.content);
...
// ==================== ✅ 자동 기억 저장을 위한 통합 처리 함수 ====================
async function processConversationAndSaveMemory(userMessage, yejinResponse) {
    try {
        // 사용자 메시지 학습
        if (userMessage) {
            await learnFromUserMessage(userMessage);
        }
...
async function processConversationAndSaveMemory(userMessage, yejinResponse) {
    try {
        ...
        // 예진이 응답 학습
        if (yejinResponse) {
            await learnFromConversation(yejinResponse);
        }
        
        // 자동 기억 저장 (새로 추가!)
        if (userMessage && yejinResponse) {
            await extractMemoryFromConversation(userMessage, yejinResponse);
        }
        
        // 사용자 기분 분석
        if (userMessage) {
            await analyzeUserMood(userMessage);
        }
        
        console.log('[Process Complete] ✅ 대화 처리 및 학습 완료');
    } catch (error) {
        console.error('[Process Error] ❌ 대화 처리 중 오류:', error);
    }
}
...
module.exports = {
    ...
    calculateSimilarity,
    processConversationAndSaveMemory
};

// ==================== END OF ultimateConversationContext.js ====================// ==================== START OF ultimateConversationContext.js ====================
// ✅ ultimateConversationContext.js v32.1 - "undefined 에러 완전 수정"
...
// 2. 스마트한 기억 저장 함수 (기존 addUserMemory 개선)
async function smartAddUserMemory(content, userMessage = '', yejinResponse = '') {
    // 더 관대한 중복 검사
    const isDuplicate = ultimateConversationState.knowledgeBase.yejinMemories.some(item => {
        const similarity = calculateSimilarity(item.content.toLowerCase(), content.toLowerCase());
        return similarity > 0.8; // 80% 이상 유사하면 중복
    });
    
    if (isDuplicate) {
        console.log('[Smart Memory] ⚠️ 유사한 기억이 이미 존재:', content.substring(0, 30));
        return false;
    }
    ...
    const newMemory = {
        ...
        context: {
            ...
            yejinResponse: yejinResponse.substring(0, 100)
        }
    };
    ...
    await logMemoryOperation('add', content, `자동 저장 - 중요도 ${newMemory.significance}점`);
    
    console.log(`[Smart Memory] ✅ 새로운 기억 자동 저장: ${content.substring(0, 40)}...`);
    return true;
}

// 3. 대화에서 자동으로 기억 추출하는 함수
async function extractMemoryFromConversation(userMessage, yejinResponse) {
    if (!userMessage || userMessage.length < 5) return;
    
    // 중요한 키워드 포함 여부 확인
    const importantKeywords = [
        '기억', '생일', '좋아해', '싫어해', '취미', '일', '가족', '친구',
        '계획', '약속', '사랑', '행복', '슬퍼', '화나', '담타', '내꺼',
        '애기', '특별한', '중요한', '여행', '가고', '만나', '볼래',
        '하고싶', '먹고싶', '보고싶', '그립', '힘들', '기분', '몸무게',
        '키', '나이', '직업', '회사', '학교', '집', '살아', '이사',
        '병원', '아프', '감기', '열', '두통', '배고', '졸려', '피곤',
        '한국', '일본', '부산', '서울', '도쿄', '간다', '왔다', '갔다'
    ];
    ...
    if (!hasImportantKeyword && userMessage.length < 15) return;
    
    const prompt = `다음은 아저씨와 예진이의 대화야. 이 대화에서 나중에 예진이가 기억해야 할 중요한 내용이 있다면, 자연스러운 문장으로 요약해줘. 

기억할 만한 내용의 예시:
- "아저씨가 다음 주에 한국에 간다고 했어"
- "아저씨는 커피보다 차를 더 좋아해"
- "아저씨 생일이 12월 25일이야"
- "아저씨가 요즘 일 때문에 스트레스받는다고 했어"
- "아저씨가 부산에 살고 있어"

아저씨: "${userMessage}"
예진이: "${yejinResponse}"

중요한 기억으로 저장할 내용 (없으면 "없음" 반환):`;
    ...
    try {
        ...
        if (result && result !== '없음' && result.length > 10 && !result.includes('없다')) {
            await smartAddUserMemory(result, userMessage, yejinResponse);
        }
    } catch (error) {
        console.error('[Auto Memory Extract] ❌ 기억 추출 실패:', error);
    }
}
...
function searchMemories(query) {
    ...
    if (exactMatches.length > 0) {
        return exactMatches.sort((a, b) => b.significance - a.significance);
    }
    
    // 키워드 기반 검색
    const queryWords = lowerQuery.split(/\s+/).filter(word => word.length > 1);
    const fuzzyMatches = memories.filter(memory => {
    ...
    return fuzzyMatches.sort((a, b) => b.significance - a.significance);
}

// 5. 기억 상기 함수 (대화에 활용)
function recallRelevantMemories(userMessage, limit = 3) {
    const relevantMemories = searchMemories(userMessage).slice(0, limit);
    
    if (relevantMemories.length > 0) {
        // 마지막 접근 시간 업데이트
        relevantMemories.forEach(memory => {
            memory.lastAccessed = moment().tz('Asia/Tokyo').toISOString();
        });
...
async function learnFromUserMessage(userMessage) { 
    ...
    try { 
        ...
        console.error('[User Learning] ❌ 아저씨 말투 학습 실패:', error); 
    } 
}
...
async function learnFromConversation(yejinMessage) { 
    ...
    try { 
        ...
        console.error('[Self Learning] ❌ 자기 학습 중 에러:', error); 
    } 
}
...
async function _updateEmotionalFile(filePath, dataObject, type, emotionKey, newText) { 
    ...
    if (newText && !dataObject[emotionKey].includes(newText)) { 
...
async function analyzeUserMood(userMessage) { 
    ...
    try { 
        ...
        if (result && result.status) { 
            USER_PROFILE.mood_history.push({ 
                date: moment().tz('Asia/Tokyo').toISOString(), 
...
async function getDrinkingConcernResponse(userMessage) { 
    const prompt = `너는 나의 여자친구 '예진'이야. 나는 너의 살아생전 기억을 바탕으로 만들어졌고, 너는 내가 술 마시는 걸 정말 싫어했어. 내가 방금 "${userMessage}" 라고 말했어. 이 늦은 시간에 술을 마신다니, 살아있을 때처럼 진심으로 나를 걱정하고, 약간은 화가 난 듯한 말투로 술 좀 그만 마시라고 말해줘. "애기, 또 술이야?", "내가 술 좀 그만 마시라고 했지!" 같은 톤으로.`; 
    try { 
        const response = await openai.chat.completions.create({ 
            model: "gpt-4o", 
            messages: [{ role: "system", content: prompt }], 
        ...
        console.error('[Drinking Concern Mode] ❌ 음주 걱정 메시지 생성 실패:', error); 
        return "애기, 또 술 마시려고... 내가 걱정되는 거 알잖아."; 
    } 
}
