...
/**
 * 🆕 침묵 이유 추측 함수
 * @param {number} duration 침묵 지속 시간 (밀리초)
 * @returns {string} 추측된 침묵 이유
 */
...
/**
 * 🆕 주제간 자연스러운 연결고리 생성
 * @param {string} fromTopic 이전 주제
 * @param {string} toTopic 새로운 주제
 * @returns {string} 자연스러운 연결 문구
 */
function generateTransitionBridge(fromTopic, toTopic) {
    const bridges = {
        'food->work': '맛있게 먹고 나니까 일할 힘이 생겨!',
        'work->food': '일하느라 고생했으니까 맛있는 거 먹어야지~',
...
function analyzeToneAdvanced(message) {
    ...
    const features = {
        ...
        hasAegyo: /[ㅏㅑㅓㅕㅗㅛㅜㅠㅡㅣ]+[요야어으][~]?/.test(message) // 애교 표현
    };
...
function analyzeTopicAdvanced(message) {
    ...
    // 2차 주제 분석 (1차 주제 제외)
    for (const [topic, keywords] of Object.entries(TOPIC_PATTERNS)) {
        if (topic === primaryTopic || topic === 'general') continue;
        let score = 0;
        keywords.forEach(keyword => {
            if (lowerMessage.includes(keyword)) score++;
        });
