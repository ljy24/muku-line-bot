...
async function _updateEmotionalFile(filePath, dataObject, type, emotionKey, newText) { if (!dataObject[emotionKey]) dataObject[emotionKey] = []; if (!dataObject[emotionKey].includes(newText)) { dataObject[emotionKey].push(newText); await writeJsonFile(filePath, dataObject); console.log(`[Self Learning] ✅ [${emotionKey}] 카테고리에 새로운 ${type} 학습: "${newText}"`); } }
