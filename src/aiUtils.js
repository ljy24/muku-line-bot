// ✅ src/aiUtils.js v2.0 - saveLog 기능 추가

const { OpenAI } = require('openai');
const fs = require('fs').promises; // 로그 저장을 위해 fs/promises 사용
const path = require('path');
require('dotenv').config();

const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
const LOG_FILE_PATH = path.join(process.cwd(), 'log.json'); // 로그 파일 경로

/**
 * [추가] 대화 내용을 log.json 파일에 저장하는 공용 함수
 * @param {string} speaker 말하는 사람 ('아저씨' 또는 '예진이')
 * @param {string} message 대화 내용
 */
...
function cleanReply(reply) {
    ...
    if (!cleaned || cleaned.length < 2) return '응? 다시 말해봐 아저씨';
    return cleaned;
}
