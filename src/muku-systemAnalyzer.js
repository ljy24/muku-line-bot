...
async function initializeCoreMemorySystems(modules, client) {
    ...
    if (modules.unifiedConflictManager && modules.unifiedConflictManager.initializeMukuUnifiedConflictSystem) {
        try {
            console.log(`${colors.conflict}💥 [갈등 초기화] unifiedConflictManager 초기화 시작...${colors.reset}`);
            await modules.unifiedConflictManager.initializeMukuUnifiedConflictSystem();
