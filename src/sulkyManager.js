...
function startDamtaProgressiveReconcile() {
    ...
    // 🚬 삐짐 레벨에 따른 간단한 템플릿 응답!
    let responseArray;
    
    if (sulkyState.sulkyLevel <= 1) {
        // 안 삐졌거나 살짝 삐졌을 때 - 신나는 반응 35개
        responseArray = [
            "오~ 담타 ㄱㄱㄱ!",
            "담타다 담타! ㄱㄱㄱ",
            "좋아! 담타 가자아~",
    ...
    console.log(`🚬 [담타간단응답] 선택된 응답: "${selectedResponse}"`);
    
    // 🚬 템플릿 응답으로 바로 반환 (복잡한 context 없이!)
    return {
        type: 'text',
        comment: selectedResponse,
        damtaTriggerSuccess: true,
        damtaStarted: true,
        isTemplateResponse: true,  // 템플릿 응답임을 표시
