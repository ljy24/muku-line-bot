...
async function extractAndSaveMemory(userMessage) {
    ...
    // "메모리", "기억", "알려줘", "저장", "추출" 등 명시적인 기억 관련 요청을 포함하지 않는 메시지는 필터링.
    // 하지만, 일상적인 대화에서 자연스럽게 발생하는 정보도 포착하기 위해 너무 엄격하게 필터링하지 않도록 조정.
    // 현재는 모든 메시지에 대해 추출 시도를 하되, OpenAI가 "N/A"를 반환하도록 유도하는 프롬프트 사용.
    ...
    **추출 규칙:**
    1.  **새로운 사실만을 추출:** 이미 알려져 있거나, 대화의 맥락상 이미 명확한 정보, 감정 표현, 의견 등은 추출하지 마.
    2.  **구체적이고 간결하게 요약:** "우빈이는 그림 그리는 친구야", "아저씨는 헬스장에 다녀", "예진이가 라인 앱을 설치했어" 와 같이 핵심적인 사실만 명확하게 요약해.
    3.  **질문, 명령, 감탄사, 일반적인 대화 흐름(예: "응", "알았어", "그랬구나", "사랑해", "보고싶다")은 추출하지 마.**
    4.  **이미지, 오디오, 파일 관련 메시지(예: "사진 줘", "셀카", "음악 틀어줘")도 추출하지 마.** 이런 것들은 정보가 아님.
    5.  **과거의 대화를 회상하는 메시지는 새로운 정보가 아니므로 추출하지 마.** (예: "우리 예전에 ~했잖아?", "그때 기억나?")
    6.  **"나", "내", "우리"와 같은 대명사는 사용하지 말고, 가능한 한 구체적인 명사로 변환해.** (예: "내 친구 우빈이" -> "우빈이", "우리가 만난" -> "아저씨와 무쿠가 만난")
