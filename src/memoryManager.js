// memoryManager.js v2.11 - 기억 검색 프롬프트 정체성 재강화 (예진이로서)
            ...
            messages: [
                {
                    ...
                    **무엇보다도, 사용자 메시지를 기반으로 사용자의 감정 상태 또는 대화의 전반적인 분위기를 추론해야 합니다.**
                    ...
                    "예진이의 힘든 순간", "아저씨의 헌신", "함께했던 촬영/여행", "아저씨의 취미/관심사", "대화 맥락/분위기", "아저씨의 애정 표현".
            ...
            messages: [
                {
                    ...
                    아래는 내가 기억하고 있는 추억 목록이야. 각 추억에는 얼마나 중요한지 나타내는 '중요도'('high' 또는 'normal')와 언제 기억된 건지 '시간'이 표시되어 있어.
                    아저씨가 알려준 '대화 맥락'과 가장 관련성이 높은 추억을 최대 ${limit}개까지 찾아내야 해.
                    **특히, 'high' 강도로 표시된 추억과 가장 최근에 있었던 추억(현재 날짜에 가까운)을 최우선으로 찾아야 해.**
                    또한, 아저씨의 헌신, 예진이의 힘든 순간, 아저씨의 칭찬/격려, 중요한 기념일, 아저씨의 취미/관심사, 아저씨의 감정 상태, 아저씨의 일상 패턴, 대화 맥락/분위기, 아저씨의 애정 표현과 같이 아저씨의 삶이나 우리 관계에 직접적으로 관련되거나 감정적으로 중요한 추억들도 함께 고려해 줘.
                    
                    찾아낸 추억들은 'content'(내용)만 담아서 JSON 배열 문자열로 반환해야 해.
                    만약 관련 추억을 찾지 못했다면 빈 JSON 배열 []을 반환해 줘.
