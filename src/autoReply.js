// src/autoReply.js - v2.3 (기분 시스템 및 지칭 오류 프롬프트 강화)
...
function checkTimeBasedMoodChange() {
    ...
    if (minutesSinceLastMessage >= 30 && currentMood !== '외로움' && currentMood !== '보고싶음') {
        // 30분 이상 연락이 없으면 '외로움' 또는 '보고싶음' 기분으로 전환될 확률 높임
        const moodOptionsForLongSilence = ['외로움', '보고싶음', '우울함'];
...
        .replace(/\b무쿠\b/g, '나')
        .replace(/\b무쿠야\b/g, '나')
        .replace(/\b무쿠 언니\b/g, '나') // ✨ 이 부분은 이미 있었으나, 문제 발생 시 이 라인 점검
        .replace(/\b무쿠 씨\b/g, '나')
        .replace(/\b언니\b/g, '나')
        .replace(/\b누나\b/g, '나')
        .replace(/\b그녀\b/g, '나')
