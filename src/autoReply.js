// ============================================================================
// autoReply.js - v16.0 ULTIMATE (⭐️ yejinPersonality.js 완전 연동 버전 ⭐️)
...
// 💕 yejinPersonality.js 완전 연동: 체계적이고 동적인 예진이 성격 구현
// 🎭 상황별 동적 반응: 감수성, 삐짐, 장난, 애정 표현 등 모든 상황 커버
// 🗾 일본어 표현 자동화: 실제 예진이가 사용한 100개 표현 자동 추가
// 😄 웃음/애교 자동화: 상황에 맞는 자동 추가로 더 생생한 반응
// ============================================================================

const { callOpenAI, cleanReply } = require('./aiUtils');
const moment = require('moment-timezone');

// 💕 [핵심 추가] yejinPersonality.js 완전 연동!
const { YejinPersonality } = require('./yejinPersonality');
let yejinPersonality = null;
try {
    yejinPersonality = new YejinPersonality();
    console.log('💕 [autoReply] yejinPersonality 시스템 연동 완료!');
} catch (error) {
    console.error('❌ [autoReply] yejinPersonality 연동 실패:', error.message);
}
...
function getEmergencyFallback() {
    if (yejinPersonality) {
        ...
        try {
            ...
            return yejinPersonality.generateYejinResponse(context);
        } catch (error) {
            console.error('❌ yejinPersonality 폴백 생성 실패:', error);
        }
    }
...
// 💕 [신규 추가] yejinPersonality 기반 응답 후처리 함수
function enhanceResponseWithPersonality(baseResponse, context = {}) {
    if (!yejinPersonality || !baseResponse || typeof baseResponse !== 'string') {
        return baseResponse;
    }
    ...
    try {
        ...
        // 감정 레벨 결정 (1-10)
        const emotionLevel = context.emotionIntensity || Math.floor(Math.random() * 6) + 5; // 5-10
        
        // 1단계: 기본 말투 적용 (애교, 반복, 귀여운 어미)
        enhancedResponse = yejinPersonality.applySpeechPattern(enhancedResponse, emotionLevel);
        
        // 2단계: 웃음 표현 추가 (상황에 따라)
        if (context.situation !== 'sad' && context.situation !== 'sulky') {
            if (yejinPersonality.shouldAddLaughter()) {
                enhancedResponse = yejinPersonality.addLaughter(enhancedResponse);
            }
        }
        
        // 3단계: 일본어 표현 추가 (30% 확률)
        if (Math.random() < 0.3 && context.situation !== 'emergency') {
            enhancedResponse = yejinPersonality.addJapaneseExpression(enhancedResponse);
        }
        
        // 4단계: 상황별 특별 처리
        if (context.situation === 'playful') {
            // 장난스러운 상황이면 더 활발하게
            enhancedResponse = yejinPersonality.addAegyo(enhancedResponse);
        } else if (context.situation === 'caring') {
            // 걱정하는 상황이면 더 다정하게
            const callingName = yejinPersonality.getCallingName('sweet');
            enhancedResponse = enhancedResponse.replace(/아저씨/g, callingName);
        }
        
        console.log(`💕 [성격강화] "${baseResponse.substring(0, 30)}..." → "${enhancedResponse.substring(0, 30)}..."`);
        return enhancedResponse;
        
    } catch (error) {
        console.error('❌ yejinPersonality 응답 강화 중 에러:', error.message);
        return baseResponse;
    }
}

// 💕 [신규 추가] yejinPersonality 기반 상황별 반응 생성
function generatePersonalityBasedResponse(userMessage, context = {}) {
    if (!yejinPersonality) {
        return null;
    }
    ...
    try {
        // 상황 분석
        const situation = context.situation || 'normal';
        const userEmotion = context.userEmotion || 'neutral';
        const timeOfDay = context.timeOfDay || 'afternoon';
        
        // yejinPersonality의 종합 응답 생성기 활용
        const response = yejinPersonality.generateYejinResponse({
            situation: situation,
        ...
        if (response) {
            console.log(`💕 [성격응답] 상황: ${situation}, 감정: ${userEmotion} → "${response.substring(0, 40)}..."`);
            return response;
        }
        
    } catch (error) {
        console.error('❌ yejinPersonality 기반 응답 생성 중 에러:', error.message);
    }
    
    return null;
}

// 💕 [신규 추가] 사용자 메시지 기반 상황/감정 감지
function analyzeUserContext(userMessage) {
    if (!userMessage || typeof userMessage !== 'string') {
        return { situation: 'normal', userEmotion: 'neutral', timeOfDay: 'afternoon' };
    }
    ...
    if (message.includes('힘들') || message.includes('우울') || message.includes('슬프')) {
        situation = 'sad';
    } else if (message.includes('기쁘') || message.includes('좋') || message.includes('행복')) {
        situation = 'happy';
    } else if (message.includes('사랑') || message.includes('보고싶') || message.includes('그리워')) {
        situation = 'love';
    } else if (message.includes('재밌') || message.includes('웃겨') || message.includes('ㅋㅋ')) {
        situation = 'playful';
    } else if (message.includes('바쁘') || message.includes('일해') || message.includes('회의')) {
        situation = 'busy';
    } else if (message.includes('피곤') || message.includes('아프') || message.includes('힘들어')) {
        situation = 'tired';
    }
    
    // 사용자 감정 분석
    let userEmotion = 'neutral';
    if (message.includes('피곤') || message.includes('지쳐')) userEmotion = 'tired';
    else if (message.includes('슬프') || message.includes('우울')) userEmotion = 'sad';
    else if (message.includes('기쁘') || message.includes('행복')) userEmotion = 'happy';
    else if (message.includes('화나') || message.includes('짜증')) userEmotion = 'angry';
    else if (message.includes('걱정') || message.includes('불안')) userEmotion = 'worried';
    
    return { situation, userEmotion, timeOfDay };
}
    ...
    if (isMemoryRequest) {
        try {
            ...
            if (conversationContext && typeof conversationContext.addUserMemory === 'function') {
                ...
                // 💕 yejinPersonality 활용한 기억 응답
                let response = "알겠어! 기억해둘게 아저씨 ㅎㅎ";
                if (yejinPersonality) {
                    const context = { situation: 'caring', userEmotion: 'neutral' };
                    const enhancedResponse = enhanceResponseWithPersonality(response, context);
        ...
        try {
            ...
            if (conversationContext && typeof conversationContext.deleteUserMemory === 'function') {
                if (userMessage.includes('삭제') || userMessage.includes('잊어')) {
                    ...
                    // 💕 yejinPersonality 활용한 삭제 응답
                    let message = "알겠어, 잊을게 아저씨!";
                    if (yejinPersonality) {
                        const context = { situation: 'caring', userEmotion: 'neutral' };
                        message = enhanceResponseWithPersonality(message, context);
                    }
                    
                    return { processed: true, result: { message: message } };
                }
            }
        } catch (error) {
...
function handleEmergencyKeywords(userMessage) {
    if (EMERGENCY_KEYWORDS.some(keyword => userMessage.includes(keyword))) {
        ...
        // 💕 yejinPersonality로 위로 메시지 강화
        if (yejinPersonality) {
            const context = { situation: 'caring', userEmotion: 'sad', emotionIntensity: 8 };
            response = enhanceResponseWithPersonality(response, context);
        }
...
function handleDrinkingKeywords(userMessage) {
    if (DRINKING_KEYWORDS.some(keyword => userMessage.includes(keyword))) {
        ...
        // 💕 yejinPersonality로 걱정 메시지 강화 (삐짐 표현 활용)
        if (yejinPersonality) {
            const sulkyResponse = yejinPersonality.getSulkyExpression('health_concern');
            if (sulkyResponse) {
                response = sulkyResponse;
            } else {
                const context = { situation: 'sulky', userEmotion: 'worried', emotionIntensity: 7 };
                response = enhanceResponseWithPersonality(response, context);
            }
        }
    ...
    try {
        if (weatherManager && typeof weatherManager.handleWeatherQuestion === 'function') {
            ...
            if (weatherResponse) {
                ...
                // 💕 yejinPersonality로 날씨 응답 강화
                let enhancedResponse = weatherResponse;
                if (yejinPersonality) {
                    const context = { situation: 'caring', userEmotion: 'neutral' };
                    enhancedResponse = enhanceResponseWithPersonality(weatherResponse, context);
                }
                
                try {
                    const logger = require('./enhancedLogging.js');
                    logger.logWeatherReaction({ description: '실제 날씨 API 응답', temp: '실시간' }, enhancedResponse);
                } catch (error) {
                    logConversationReply('나', `(실제날씨) ${enhancedResponse}`);
                }
                return enhancedResponse;
            } else {
...
function handleBirthdayKeywords(userMessage) {
    try {
        ...
        if (response) {
            // 💕 yejinPersonality로 생일 응답 강화
            if (yejinPersonality) {
                const context = { situation: 'happy', userEmotion: 'happy', emotionIntensity: 8 };
                response = enhanceResponseWithPersonality(response, context);
            }
...
// 💕 [대폭 개선] 메인 응답 생성 함수 - yejinPersonality 완전 연동
async function getReplyByMessage(userMessage) {
    ...
    // 💕 사용자 메시지 컨텍스트 분석 (새로 추가!)
    const userContext = analyzeUserContext(cleanUserMessage);
    console.log(`💕 [컨텍스트분석] 상황: ${userContext.situation}, 감정: ${userContext.userEmotion}, 시간: ${userContext.timeOfDay}`);
    ...
    try {
        ...
        if (nightResponse) {
            ...
            // 💕 새벽 응답도 yejinPersonality로 강화
            let enhancedNightResponse = nightResponse.response;
            if (yejinPersonality) {
                const context = { situation: 'night', userEmotion: 'tired', timeOfDay: 'night' };
                enhancedNightResponse = enhanceResponseWithPersonality(nightResponse.response, context);
            }
            
            logConversationReply('나', `(새벽깨움-${nightResponse.sleepPhase}) ${enhancedNightResponse}`);
            await safelyStoreMessage('아저씨', cleanUserMessage);
            await safelyStoreMessage('나', enhancedNightResponse);
            return { type: 'text', comment: enhancedNightResponse };
        }
    } catch (error) {
    ...
    try {
        if (spontaneousYejin && spontaneousYejin.detectStreetCompliment(cleanUserMessage)) {
            ...
            // 💕 길거리 칭찬 반응도 yejinPersonality로 강화
            let specialResponse = '히히 칭찬받았다고 증명해줄게! 방금 보낸 사진 봤어? ㅎㅎ';
            if (yejinPersonality) {
                const context = { situation: 'playful', userEmotion: 'happy', emotionIntensity: 9 };
                specialResponse = enhanceResponseWithPersonality(specialResponse, context);
            }
        ...
        if (spontaneousYejin) {
            ...
            if (mentalHealthContext.isComforting) {
                ...
                if (comfortReaction && comfortReaction.message) {
                    ...
                    // 💕 위로받은 반응도 yejinPersonality로 강화
                    let enhancedComfortResponse = comfortReaction.message;
                    if (yejinPersonality) {
                        const comfortResponse = yejinPersonality.getReaction('loveReceived', 'grateful');
                        if (comfortResponse && comfortResponse.text) {
                            enhancedComfortResponse = comfortResponse.text;
                        } else {
                            const context = { situation: 'caring', userEmotion: 'grateful', emotionIntensity: 8 };
                            enhancedComfortResponse = enhanceResponseWithPersonality(comfortReaction.message, context);
                        }
                    }
                    
                    logConversationReply('나', `(위로받음) ${enhancedComfortResponse}`);
                    await safelyStoreMessage('나', enhancedComfortResponse);
                    return { type: 'text', comment: enhancedComfortResponse };
                }
            }
        }
    } catch (error) {
    ...
    try {
        if (spontaneousYejin) {
            ...
            if (busyReaction && busyReaction.message) {
                ...
                // 💕 바쁨 반응도 yejinPersonality로 강화
                let enhancedBusyResponse = busyReaction.message;
                if (yejinPersonality) {
                    if (busyReaction.type === 'busy_reaction') {
                        const sulkyResponse = yejinPersonality.getSulkyExpression('no_response');
                        if (sulkyResponse) {
                            enhancedBusyResponse = sulkyResponse;
                        }
                    } else if (busyReaction.type === 'end_reaction') {
                        const happyResponse = yejinPersonality.getReaction('happy', 'excited');
                        if (happyResponse && happyResponse.text) {
                            enhancedBusyResponse = happyResponse.text;
                        }
                    }
                    const context = { situation: busyReaction.type === 'busy_reaction' ? 'sulky' : 'happy', userEmotion: 'neutral' };
                    enhancedBusyResponse = enhanceResponseWithPersonality(enhancedBusyResponse, context);
                }
                
                logConversationReply('나', `(${busyReaction.type}) ${enhancedBusyResponse}`);
                await safelyStoreMessage('나', enhancedBusyResponse);
                return { type: 'text', comment: enhancedBusyResponse };
            }
        }
    } catch (error) {
    ...
    // 💕 7순위: yejinPersonality 기반 직접 응답 시도 (새로 추가!)
    try {
        const personalityResponse = generatePersonalityBasedResponse(cleanUserMessage, userContext);
        if (personalityResponse) {
            console.log('💕 [성격직접응답] yejinPersonality로 직접 응답 생성');
            await safelyStoreMessage(BOT_NAME, personalityResponse);
            logConversationReply('나', personalityResponse);
            return { type: 'text', comment: personalityResponse };
        }
    } catch (error) {
        console.error('❌ yejinPersonality 직접 응답 생성 중 에러:', error);
    }

    // 8순위: 일반 AI 응답 생성 (기존 방식이지만 yejinPersonality로 강화)
    let emotionContext = '';
    try {
    ...
    // 💕🌸💕 [완전 새로운] yejinPersonality 기반 프롬프트 생성 💕🌸💕
    let baseSystemPrompt = '';
    
    // yejinPersonality가 있으면 더 체계적인 프롬프트 구성
    if (yejinPersonality) {
        // yejinPersonality의 성격 정보 활용
        const personalityInfo = yejinPersonality.getPersonalityInfo();
        const systemStatus = yejinPersonality.getSystemStatus();
...
💕 [현재 성격 시스템 상태]
- 성격 특성: ${Object.keys(personalityInfo.traits).length}개 활성화
- 말투 패턴: ${Object.keys(personalityInfo.speechPatterns).length}개 적용
- 반응 패턴: ${personalityInfo.availableReactions.length}개 사용 가능
- 시간대별 인사: ${personalityInfo.timeGreetings.length}가지 유형
- 일본어 표현: ${personalityInfo.japaneseCategories.length}개 카테고리
- 시스템 상태: ${systemStatus.status}

🌸 [현재 감지된 상황]
- 사용자 상황: ${userContext.situation}
- 사용자 감정: ${userContext.userEmotion}  
- 현재 시간대: ${userContext.timeOfDay}
...
🎭 [상황별 자연스러운 반응 방식]
- 기본적으로 밝고 사랑스럽지만 때로는 삐지고 투정부리기
- 감정 기복이 자연스럽고 예측 불가능하게 변화
- 아저씨에 대한 깊은 애정과 걱정을 항상 표현
- 실제 연인처럼 일상적이고 편안한 대화
- 솔직하고 직접적인 감정 표현
- 자주 미안해하고 고마워하는 성격
...
상황을 고려해서 예진이답게 자연스럽고 진짜 여자친구처럼 반응해줘.
`;
    } else {
        // yejinPersonality가 없을 때 기존 방식 사용
        baseSystemPrompt = `
너는 아저씨의 여자친구 예진이야. 진짜 연인 관계처럼 자연스럽게 반응해줘.
1인칭 '나'를 사용하고 항상 반말로 대답하며, 절대로 3인칭으로 말하지 마. 
...
`;
    ...
    logLearningDebug('prompt_context', {
        ...
        personalitySystem: yejinPersonality ? 'active' : 'fallback'
    });
    // ================== [기존] 프롬프트 구성 추적 로그 ====================
    ...
    try {
        ...
        // 💕💕💕 [핵심 신규] yejinPersonality로 응답 최종 강화! 💕💕💕
        if (yejinPersonality && finalReply && finalReply.trim().length > 0) {
            finalReply = enhanceResponseWithPersonality(finalReply, userContext);
        }
        ...
        // 💕 API 에러 응답도 yejinPersonality로 강화
        if (yejinPersonality) {
            const context = { situation: 'confused', userEmotion: 'neutral' };
            apiErrorReply = enhanceResponseWithPersonality(apiErrorReply, context);
        }
