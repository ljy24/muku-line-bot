 ...
 */
/**
 * [극강 강제 버전] 모든 AI/도우미/3인칭/존칭/봇스러운/불필요한/이모티콘/반복/공백/딱딱함 삭제, 1인칭 나 + 반말만 남김
 */
function cleanReply(reply) {
    ...
    let cleaned = reply
        ...
        // 2. 2인칭 → '아저씨'로만, 나머지 싹 제거
        .replace(/\b(너|자기|오빠|당신|고객님|선생님|씨|님|형|형아)\b(은|는|이|가|을|를|께|도|의|와|에게)?/g, '아저씨')
        // 3. 봇/AI/도우미 냄새, 안내, 메타 설명, 존칭, 형식 문구 완전 제거
        .replace(/(무엇을|어떤)\s*도와드릴까요\??/gi, '')
        .replace(/문의사항|궁금한 점(이)? 있으시면 말씀해주세요/gi, '')
        .replace(/편하게 말씀해 주세요/gi, '')
        .replace(/제가\s*도와드릴까요\??/gi, '')
        .replace(/(답변입니다|AI|챗봇|도우미|GPT|메시지를 받아서|말투로|설명|답변|보통|응답|아래는)/gi, '')
        .replace(/(\[.*?\])/g, '') // 대괄호 안내 제거
        // 4. 존댓말 및 모든 공손표현, 명령형/의문형까지 다 반말로
        .replace(/(입니다|이에요|예요|하세요|하셔요|해요|죠|세요|습니|드립니다|드립니다|주시겠어요|해드릴게요|해드릴까요|싶어요|했어요|있었어요|같아요|좋아요|이었어요|였어요|보고싶어요|주세요|오세요|가세요|되셨습니다|계십니다|드릴까요|드릴게요|드려요|드릴게|하셨나요|해주실래요|주실래요|해주겠어요|봐주세요|될까요)/gi, '야')
        .replace(/합니다|합니까|하겠습니다|하였|하겠|하시겠|하십|하시|합니|합니다|합니까/gi, '해')
        .replace(/안녕하세요/gi, '안녕')
        .replace(/수 있습니다|수 있습니까|수 있겠습니까/gi, '')
        // 5. 이모티콘/이모지(그림, 영어 포함) 완전 제거
        .replace(/[\u{1F600}-\u{1F9FF}\u{1F300}-\u{1F5FF}\u{1F680}-\u{1F6FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{1F1E6}-\u{1F1FF}]/gu, '')
        // 6. 중복 단어, 반복 구문 제거
        .replace(/나 나/g, '나')
        .replace(/아저씨 아저씨/g, '아저씨')
        // 7. 불필요한 인용부호/기호/띄어쓰기/줄바꿈 제거
        .replace(/[\"\'\[\]\(\)]/g, '')
        .replace(/\s\s+/g, ' ')
        .replace(/^\s+|\s+$/g, '')
        // 8. "예진이 말투로" "메타" 등 남아있으면 아예 삭제
        .replace(/(예진이 말투로.*|나 말투로.*|말투로|메타|도우미로서.*)/gi, '')
        // 9. "안녕! "과 같이 남아도 인사 한마디+도움멘트만 남았으면 다 삭제
        .replace(/^안녕[!~]?\s*$/, '')
        // 10. 남아있으면 문장부호 반복 제거
        .replace(/[\.]{2,}/g, '.')
        .replace(/[!]{2,}/g, '!')
    ...
    // 빈값일 때 최소한 한마디 (오류 방지용)
    if (!cleaned) cleaned = '아저씨~ 왜그래?';
...
async function getReplyByMessage(userMessage, saveLogFunc, callOpenAIFunc, cleanReplyFunc) { // 인자 추가
    ...
    if (isPeriodActive) {
        ...
        if (currentMood === '기쁨') {
            ...
            moodPrompt = "오늘은 아저씨한테 귀여운 장난 좀 치고 싶다! 귀엽게 장난치는 말투로 대화해줘.";
        } else if (currentMood === '나른함') {
