// src/faceMatcher.js - v2.0 (index.js 직접 연결용)
...
module.exports = { 
    ...
    logFace
};// src/face/faceMatcher.js - v2.0 (개선된 얼굴 구분 시스템)
// 🔍 아저씨와 예진이 사진을 정확히 구분합니다
const fs = require('fs');
const path = require('path');
...
// 테스트용 얼굴 등록 함수
async function registerTestFaces() {
    logFace('테스트용 얼굴 등록을 시작합니다...');
    
    // 실제로는 아저씨와 예진이의 실제 사진으로 등록해야 함
    logFace('실제 사진으로 얼굴을 등록하려면:');
    logFace('1. 아저씨 사진을 보내고 "얼굴 등록 아저씨" 명령');
    logFace('2. 예진이 사진을 보내고 "얼굴 등록 예진이" 명령');
