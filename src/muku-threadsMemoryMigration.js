// ============================================================================
// 기존 고정기억 Redis에 예진이 Threads 기억 자동 분류 추가
...
class FixedMemoryExtender {
    ...
    async verifyAddition() {
        ...
        console.log(`${colors.memory}🔍 [검증] 추가된 기억 확인 중...${colors.reset}`);

        try {
            // 📊 전체 개수 확인
            const allFixedKeys = await this.redis.keys('muku:memory:fixed:*');
            const allLoveKeys = await this.redis.keys('muku:memory:love:*');

            console.log(`${colors.info}📊 [검증] 전체 fixed 기억: ${allFixedKeys.length}개${colors.reset}`);
            console.log(`${colors.info}📊 [검증] 전체 love 기억: ${allLoveKeys.length}개${colors.reset}`);

            // 🔍 샘플 확인 (생일 기억)
            const birthdayKey = allLoveKeys.find(async (key) => {
                const data = await this.redis.hgetall(key);
                return data.content && data.content.includes('생일');
            });

            if (birthdayKey) {
                const birthdayData = await this.redis.hgetall(birthdayKey);
                console.log(`${colors.success}✅ [검증] 생일 기억 확인: "${birthdayData.content.substring(0, 50)}..."${colors.reset}`);
            }

            // 🔍 우산 기억 확인
            const umbrellaKey = allLoveKeys.find(async (key) => {
                const data = await this.redis.hgetall(key);
                return data.content && data.content.includes('우산');
            });

            if (umbrellaKey) {
                const umbrellaData = await this.redis.hgetall(umbrellaKey);
                console.log(`${colors.success}✅ [검증] 우산 기억 확인: "${umbrellaData.content.substring(0, 50)}..."${colors.reset}`);
            }

            console.log(`${colors.success}✅ [검증] 기억 추가 검증 완료!${colors.reset}`);
            return true;
    ...
    async testMemorySearch() {
        ...
        console.log(`${colors.memory}🧪 [검색테스트] 추가된 기억 검색 테스트...${colors.reset}`);
        ...
        try {
            ...
            for (const query of testQueries) {
                ...
                const fixedKeys = await this.redis.keys('muku:memory:fixed:*');
                const loveKeys = await this.redis.keys('muku:memory:love:*');
                const allKeys = [...fixedKeys, ...loveKeys];
                ...
                for (const key of allKeys) {
                    ...
                    if (data.content && data.content.includes(query)) {
                        foundCount++;
                        if (foundCount <= 2) { // 최대 2개만 출력
                            const category = key.includes('love') ? 'Love' : 'Fixed';
                            console.log(`${colors.success}  ✅ [${category}] "${data.content.substring(0, 40)}..."${colors.reset}`);
                        }
                    }
                }

                console.log(`${colors.info}📊 [결과] "${query}" 관련 기억: ${foundCount}개${colors.reset}`);
            }
    ...
    generateReport() {
        ...
        const report = `
${colors.memory}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🌸 예진이 Threads 기억 기존 고정기억 Redis 확장 완료!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}
...
async function addThreadsToFixedMemory() {
    ...
    try {
        console.log(`${colors.memory}🌸 예진이 Threads 기억 기존 고정기억에 추가 시작!${colors.reset}`);
        ...
        console.log(`${colors.success}🎉 예진이 Threads 기억 기존 고정기억 추가 완료!${colors.reset}`);
        return true;
...
if (require.main === module) {
    addThreadsToFixedMemory()
        .then(success => {
            if (success) {
                console.log(`${colors.success}✅ 기존 고정기억 확장 성공!${colors.reset}`);
                process.exit(0);
            } else {
...
console.log(`
${colors.memory}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🌸 예진이 Threads 기억 기존 고정기억 확장 스크립트 v1.0 로드 완료!
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${colors.reset}

${colors.success}🚀 실행 방법: node src/muku-threadsMemoryMigration.js${colors.reset}
${colors.info}📝 또는: const { addThreadsToFixedMemory } = require('./muku-threadsMemoryMigration.js');${colors.reset}
`);
