// ============================================================================
// muku-diarySystem.js v8.3 - 하드코딩 제거 + 하루1개보장 강화
// 🔥 핵심 수정사항:
// 1. handleDiaryCommand() 함수에서 하드코딩 메시지 완전 제거
// 2. "일기써"와 "일기장" 동일한 결과 보장 (하루 1개)
// 3. 기존 일기 있으면 새로 생성하지 않고 표시만
// 4. 자연스러운 예진이 말투로 응답 변경
// ✅ 이제 같은 날짜에 같은 일기가 나옵니다!
// ============================================================================
...
let diarySystemStatus = {
    isInitialized: false, totalEntries: 0, lastEntryDate: null, version: "8.3",
    description: "하드코딩제거+하루1개보장강화+autoReply.js방식Memory Tape연동완전적용+감기대화반영보장+실제라인대화정확수집",
        ...
        if (lowerText.includes('일기장') || lowerText.includes('일기써')) {
            ...
            try {
                ...
                if (todayDiaries && todayDiaries.length > 0) {
                    ...
                    console.log(`${colors.autoReply}📖 [autoReply방식일기장] 오늘 일기 발견: 기존 일기 표시${colors.reset}`);
...
`;
                    ...
                    if (commandType === '일기써') {
                        response += `
💕 **오늘 하루도 아저씨와 함께해서 정말 소중했어! (${latestEntry.content.length}자)**`;
                    } else {
                        response += `
💭 **아저씨~ 오늘 일기 어때? 내 마음이 잘 담겨있지? (${latestEntry.content.length}자)**`;
                    }
                    
                    console.log(`${colors.autoReply}✅ [autoReply방식일기장] 기존 일기 표시 완료${colors.reset}`);
                    return { success: true, response: response };
                    
                } else {
                    // 🆕 일기가 없는 경우에만 새로 생성
                    console.log(`${colors.autoReply}📖 [autoReply방식일기장] 오늘 일기 없음 - 새로 생성${colors.reset}`);
...
`;
                            ...
                            response += `
🌸 **방금 전에 하루를 돌아보며 예진이답게 써봤어! 아저씨와의 소중한 시간들이 담겨있어~ (${latestEntry.content.length}자)**`;
                            
                            console.log(`${colors.autoReply}✅ [autoReply방식일기장] 새 일기 생성 후 표시 완료${colors.reset}`);
                            return { success: true, response: response };
                        }
                    }
...
`;
                    fallbackResponse += `매일 밤 22시에 자동으로 autoReply.js 방식으로 실제 대화를 반영한 일기를 써줄게~

`;
                    fallbackResponse += `다시 "일기써"라고 말하면 지금 당장 써줄 수도 있어!`;
                    
                    console.log(`${colors.autoReply}⚠️ [autoReply방식일기장] 일기 생성 실패, 자연스러운 폴백 응답${colors.reset}`);
                    return { success: true, response: fallbackResponse };
                }
                
            } catch (error) {
                console.error(`${colors.error}❌ [autoReply방식일기장] 처리 실패: ${error.message}${colors.reset}`);
                return { success: false, response: "일기장 처리 중 에러가 발생했어... 미안해!" };
            }
        }
...
`;
            response += `🔥 **v8.3 수정사항 (하드코딩 제거 + 하루1개보장 강화)**
`;
            response += `- handleDiaryCommand() 함수에서 하드코딩 메시지 완전 제거
`;
            response += `- "일기써"와 "일기장" 동일한 결과 보장 (하루 1개)
`;
            response += `- 기존 일기 있으면 새로 생성하지 않고 표시만
`;
            response += `- 자연스러운 예진이 말투로 응답 변경
`;
            response += `- "**일기장 완료!** autoReply.js..." 같은 템플릿 완전 제거`;
...
async function initializeDiarySystem() {
    try {
        console.log(`${colors.autoReply}📖 [autoReply방식일기시스템] v8.3 초기화 시작... (하드코딩제거+하루1개보장강화)${colors.reset}`);
        ...
        console.log(`${colors.autoReply}✅ [autoReply방식일기시스템] v8.3 초기화 완료! (하드코딩제거+하루1개보장강화)${colors.reset}`);
        ...
        console.log(`${colors.autoReply}🔥 v8.3 완전 적용: 하드코딩 메시지 제거 + 하루1개보장 강화${colors.reset}`);
        console.log(`${colors.autoReply}💬 "일기써" = "일기장" 동일한 결과 완벽 보장${colors.reset}`);
        console.log(`${colors.autoReply}🎯 자연스러운 예진이 말투 적용완료${colors.reset}`);
        ...
        console.log(`${colors.autoReply}🆕 "일기써" 명령어도 동일하게 작동! 하드코딩 완전 제거!${colors.reset}`);
        
        return true;
    } catch (error) {
        console.error(`${colors.error}❌ autoReply.js 방식 일기장 시스템 v8.3 초기화 실패: ${error.message}${colors.reset}`);
        ...
        diarySystemStatus.hardcodingRemoved = true;
        diarySystemStatus.oneDiaryGuaranteed = true;
...
async function generateTestDiary() {
    return {
        success: false,
        message: "v8.3에서는 테스트 일기 대신 autoReply.js 방식으로 실제 라인 대화 기반 일기만 생성합니다. 매일 밤 22시에 자동으로, 또는 '일기장' 명령어로 즉시 써드릴게요!",
