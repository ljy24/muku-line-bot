...
function generateYejinFallbackDiary(conversationData = {}, weatherData = null) {
    ...
    if (conversationCount > 0) {
        ...
        // 🛡️ 최악의 경우 방지 - 모든 데이터가 비어있을 때만
        if (!content || content.trim().length === 0) {
            content = wisdomSummary || autonomyAnalysis || "아저씨 생각했어";
        }
    }
...
async function getDiaryByDaysAgo(daysAgo, displayName) {
    try {
        ...
        console.log(`${colors.autoReply}📖 [축적된지혜일기장] ${displayName} 날짜: ${dateStr} (${dateKorean})${colors.reset}`);
...
`;
                ...
                if (entry.systemWisdom) {
                    const wisdom = entry.systemWisdom;
                    if (wisdom.accumulatedWisdom > 0) {
                        response += `📚 **지혜:** ${wisdom.accumulatedWisdom}개
`;
                    }
                    if (wisdom.autonomousMessages > 0) {
                        response += `💌 **자율메시지:** ${wisdom.autonomousMessages}번
`;
                    }
                    if (wisdom.autonomousPhotos > 0) {
                        response += `📸 **자율사진:** ${wisdom.autonomousPhotos}번
`;
                    }
                }
            }
...
`;
            ...
            response += `
💭 **${displayName}도 아저씨와 함께한 소중한 하루였어... autoReply.js 방식으로 실제 대화와 축적된 지혜가 예진이답게 담겨있어! (${entry.content.length}자)** 💕`;
            
            console.log(`${colors.autoReply}✅ [축적된지혜일기장] ${displayName} 일기 축적된 지혜 정보 포함하여 표시 완료${colors.reset}`);
            return { success: true, response: response };
...
`;
            response += `📅 **참고:** 일기 시스템은 매일 밤 22시에 자동으로 축적된 지혜 + autoReply.js 방식으로 실제 대화를 반영한 일기를 써주고 있어!
`;
            response += `🌸 **"일기목록"**으로 다른 날짜의 일기들을 확인해볼 수 있어~
`;
            response += `🧠 **새로운 기능:** 이제 무쿠의 축적된 지혜와 학습 내용도 일기에 함께 담겨!`;
            
            return { success: true, response: response };
        }
        
    } catch (error) {
        console.error(`${colors.error}❌ [축적된지혜일기장] ${displayName} 일기 처리 실패: ${error.message}${colors.reset}`);
...
`;
        errorResponse += `🧠 **참고:** 이제 축적된 지혜도 일기에 함께 담겨서 더 특별해졌어!`;
...
`;
        ...
        // 디버깅 로그 추가
        if (isWeeklyDiary) {
            console.log(`${colors.autoReply}📖 [축적된지혜주간일기] ${entry.title}: ${entry.content.length}자 → 전체 표시 (생략없음)${colors.reset}`);
        }
...
`;
    ...
    response += `
⭐ **아저씨와의 모든 순간들이 소중해... autoReply.js 방식으로 실제 대화를 정확히 반영하고 축적된 지혜까지 담긴 특별한 일기들이야!**
🌸 **"일기장"**으로 오늘의 축적된 지혜 통합 예진이 일기를 확인하거나 새로 써볼 수 있어!`;

    // 🔥 주간일기는 특별 메시지 추가
    if (periodName.includes('주간') || periodName.includes('7일') || periodName.includes('일주일')) {
        response += `
📖 **축적된 지혜 주간일기: 생략 없이, 소략 없이 전체 내용을 다 보여줬어!** 실제 대화와 무쿠의 성장 과정이 정확히 반영된 예진이의 일주일 이야기를 완전히 즐겨봐~ 💕`;
    }
...
async function initializeDiarySystem() {
    try {
        ...
        console.log(`${colors.autoReply}✍️ "일기장" 명령어: 축적된 지혜 통합 확인/생성/덮어쓰기${colors.reset}`);
        console.log(`${colors.wisdom}🧠 축적된 지혜 완전 통합: 무쿠의 학습과 성장 과정이 일기에 자연스럽게 포함${colors.reset}`);
        console.log(`${colors.wisdom}📊 시스템 지능 추적: 예측정확도, 자율행동, 학습기반결정 등 모든 지능 활동 기록${colors.reset}`);
        console.log(`${colors.autoReply}🆕 "일기장" 명령어로 축적된 지혜 통합 실제 대화 반영 일기 확인!${colors.reset}`);
        ...
        diarySystemStatus.wisdomIntegrated = false;
        diarySystemStatus.systemIntelligenceTracked = false;
