// =================================================================
// ▼▼▼ 전역 GPT 모델 관리 시스템 (index.js 최상단에 추가) ▼▼▼
// =================================================================

// globalThis는 Node.js와 브라우저 환경 모두에서 작동하는 표준 전역 객체
// 여기에 함수를 등록하면 앱 전체에서 사용 가능
const globalContext = globalThis;

let forcedModel = null;

globalContext.getModel = function() {
  return forcedModel || 'gpt-4o';
}

globalContext.setForcedModel = function(model) {
...
globalContext.checkModelSwitchCommand = function(userMessage) {
  const lowerText = userMessage.toLowerCase();
  if (lowerText.includes('버전')) {
    const currentModel = globalContext.getModel();
    let modelName;
    ...
    return `지금 무쿠는 ${modelName} 버전으로 말하고 있어! 아저씨~`;
  } else if (lowerText.includes('모델 3.5') || lowerText.includes('3.5')) {
    globalContext.setForcedModel('gpt-3.5-turbo');
    return '응! 이제 3.5버전으로 말할게! 속도가 더 빨라질 거야~';
  } else if (lowerText.includes('모델 4.0') || lowerText.includes('4.0')) {
    globalContext.setForcedModel('gpt-4o'); 
    return '알겠어! 이제 4.0버전으로 말할게! 더 똑똑해질 거야~';
  } else if (lowerText.includes('모델 자동') || lowerText.includes('자동')) {
    globalContext.setForcedModel(null);
