...
// omoide.js에서 사진 관련 응답 함수를 불러옵니다.
// 파일 구조 이미지에 따르면 omoide.js는 memory 폴더 바로 아래에 있습니다.
const { getOmoideReply } = require('./memory/omoide'); // omoide.js는 './memory/omoide' 경로로 불러옵니다.
            ...
            if (event.type === 'message') {
                ...
                if (message.type === 'text') {
                    ...
                    if (/(기억\s?보여줘|내\s?기억\s?보여줘|혹시 내가 오늘 뭐한다 그랬지\?|오늘 뭐가 있더라\?|나 뭐하기로 했지\?)/i.test(text)) {
                        try {
                            const memoryList = await getMemoryListForSharing(); // autoReply.js에서 기억 목록을 가져옴
                    ...
                    // * "기억해줘", "잊지마" 등의 일반 대화는 isCommand에서 false로 반환되므로, 여기서 기억으로 저장될 수 있습니다. *
                    if (!isCommand(text)) {
                        ...
                        console.log(`[index.js] 명령어 '${text}'는 메모리 저장에서 제외됩니다.`); // 명령어로 인식된 메시지는 저장 제외
                    }
...
const sendScheduledMessage = async (type) => {
    ...
    // * 서버 부팅 후 3분 동안은 자동 메시지 전송을 건너뜁니다. *
    if (currentTime - bootTime < 3 * 60 * 1000) {
...
cron.schedule('*/15 * * * *', async () => { // 매 15분마다 실행
    ...
    // * 서버 부팅 후 3분 이내에는 침묵 감지 체크를 건너뜁니다. *
