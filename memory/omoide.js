// memory/omoide.js v1.14 - TypeError 및 이모티콘 제거 재수정
...
function cleanReply(reply) {
    ...
    // Node.js 환경에서 \p{Emoji} 등의 유니코드 속성 정규표현식이 제대로 작동하지 않을 가능성을 고려하여,
    // 더욱 범용적인 문자 범위 제거와 함께, 한글 특유의 이모티콘(ㅋㅋㅋ, ㅎㅎ, ㅠㅠ 등)도 직접 제거합니다.
    ...
    // 더 범용적인 이모지 제거 (유니코드 속성 대신 넓은 범위의 유니코드 블록 직접 지정)
    cleaned = cleaned.replace(/[\u{1F000}-\u{1FFFF}\u{20000}-\u{2FFFF}\u{30000}-\u{3FFFF}]/gu, '').trim(); // Supplementary Planes (이모지 대부분 여기 있음)
    // 한글 자음/모음 반복형 이모티콘 제거 (ㅋㅋㅋ, ㅎㅎ, ㅠㅠ 등)
    cleaned = cleaned.replace(/(ㅋㅋ+|ㅎㅎ+|ㅠㅠ+|ㅜㅜ+|흑흑+|ㅠㅠㅠ+|ㅋㅋㅋㅋ+|하하+|흐흐+)/g, '').trim();
    // 그 외 일반적인 특수 문자 이모티콘 제거 (하트, 별, 체크 등)
    cleaned = cleaned.cleaned.replace(/[♥★☆✔✅✖❌⁉❓❕❗✨🎵🎶💔👍👎👌👏]/g, '').trim();
