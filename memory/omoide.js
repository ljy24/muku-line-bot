...
async function getOmoideReply(userMessage, saveLogFunc) {
    ...
    const keywordMappings = {
        '무쿠 셀카': '추억 무쿠 사진 모음',
        '애기 셀카': '추억 무쿠 사진 모음',
        '빠계 셀카': '추억 빠계 사진 모음', // 오타 방지
        '빠계 사진': '추억 빠계 사진 모음',
        '메이드': '흑심 24_11_08 한국 메이드복_', // '메이드' 키워드 추가
        '흑심': '흑심 24_11_08 한국 메이드복_',
        '인생네컷': '추억 인생네컷',
        '커플사진': '추억 24_02_25 한국 커플사진', // 우선순위
        '일본 사진': '일본', // 일반 일본 사진 폴더 필터링을 위함
        '한국 사진': '한국', // 일반 한국 사진 폴더 필터링을 위함
        '출사': '출사',
        '필름카메라': '필름카메라',
        '애기 필름': '필름카메라',
    ...
    if (bestMatchKey) {
        ...
        } else if (mappedValue === '일본' || mappedValue === '한국' || mappedValue === '출사' || mappedValue === '필름카메라' || mappedValue === '메이드복' || mappedValue === '커플사진' || mappedValue === '인생네컷' || mappedValue === '추억 무쿠 사진 모음' || mappedValue === '추억 빠계 사진 모음') {
            // PHOTO_FOLDERS에 직접 매핑되는 키워드 처리
