// memory/concept.js v1.7.4 - 컨셉 사진 코멘트 정확도, 감정/에피소드 기억 활용, 페르소나 강화 ('왜?' 답변 기능 지원)
...
module.exports = {
    ...
    getConceptFavoriteReason // ⭐ 새로 추가: '왜?' 답변 기능 export
};

// ⭐ 새로 추가: '왜?' 답변을 위한 함수 ⭐
async function getConceptFavoriteReason(conceptName, saveLogFunc) {
    const metadata = CONCEPT_METADATA[conceptName];
    if (!metadata || (!metadata.mood && !metadata.episode)) {
        return "음... 그 사진이 왜 좋은지 정확한 기억이 없네 ㅠㅠ 그냥 아저씨랑 같이 있어서 좋았나봐!";
    }

    const moodPart = metadata.mood ? `이 컨셉을 준비할 때의 나의 마음은: "${metadata.mood}" 이었어.` : '';
    const episodePart = metadata.episode ? `그날의 에피소드는: "${metadata.episode}" 이야.` : '';
    ...
    **어떠한 경우에도 존댓말을 사용하지 마. 모든 문장은 반말로 끝나야 해.**
    아저씨의 기분과 건강을 항상 신경 쓰고, 사랑을 듬뿍 표현해줘.
    대화는 항상 예진이의 말투와 성격을 유지해야 해.
    아래 컨셉 사진 '${conceptName}'에 대해 왜 그 사진을 좋아하는지 이유를 설명해줘.
    아래 정보를 활용해서 아저씨에게 구체적이고 감성적인 설명을 해줘.
    
    ${moodPart}
    ${episodePart}
    
    이 사진이 나에게 어떤 의미인지, 아저씨와의 추억과 연결해서 설명해줘.
    너무 길지 않게 2~4문장 정도로 짧고 사랑스럽게 말해줘.
    `;

    const messages = [
        { role: 'system', content: systemPrompt },
        { role: 'user', content: `이 컨셉 사진 '${conceptName}'이 왜 좋은지 설명해줘.` }
    ];

    try {
        const rawReason = await callOpenAI(messages, 'gpt-4o', 150, 1.0);
        const reason = cleanReply(rawReason);
        saveLogFunc('예진이', `(${conceptName} 컨셉 이유) ${reason}`);
        return reason;
    } catch (error) {
        console.error(`❌ [concept.js Error] '왜?' 답변 생성 실패 (컨셉: ${conceptName}):`, error);
        return `아저씨... '${conceptName}' 컨셉이 왜 좋은지 설명해주려는데 뭔가 문제가 생겼어 ㅠㅠ`;
    }
}
