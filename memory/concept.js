...
// 컨셉 사진 폴더별 사진 개수 데이터를 JSON 파일에서 동적으로 불러오기
let CONCEPT_FOLDERS = {
...
// concept.js 파일의 현재 위치를 기준으로 data/memory/concept-index.json 경로를 resolve
const CONCEPT_INDEX_PATH = path.resolve(__dirname, '../data/memory/concept-index.json'); 

try {
    const conceptData = fs.readFileSync(CONCEPT_INDEX_PATH, 'utf8');
    CONCEPT_FOLDERS = JSON.parse(conceptData);
    console.log(`✅ [concept.js] 컨셉 인덱스 파일(${CONCEPT_INDEX_PATH}) 성공적으로 로드됨.`);
} catch (error) {
    console.error(`❌ [concept.js Error] 컨셉 인덱스 파일 로드 실패: ${error.message}`);
    // 파일 로드 실패 시, 백업 또는 기본 데이터 사용 (선택 사항)
    // 예시: CONCEPT_FOLDERS = { "기본_폴더": 10, ... };
}
...
/**
 * URL 경로에 있는 특수 문자를 인코딩하여 안전한 URL을 생성합니다.
...
function encodeImageUrl(url) {
    try {
        ...
        // pathname의 각 세그먼트를 개별적으로 디코딩 후 인코딩하여 이중 인코딩 방지
...
/**
 * 주어진 폴더 이름과 인덱스에 해당하는 컨셉 사진 URL을 생성합니다.
 * @param {string} folderName - 컨셉 사진 폴더 이름
 * @param {number|null} targetIndex - 특정 사진 인덱스 (null이면 랜덤)
 * @returns {string|null} 생성된 사진 URL 또는 null
 */
...
/**
 * 사용자 메시지에 따라 컨셉 사진 응답을 생성합니다.
 * @param {string} userMessage - 사용자 입력 메시지
 * @param {object} conversationContext - 대화 컨텍스트 (감정 상태 포함)
 * @returns {object|null} 이미지 응답 객체 또는 텍스트 응답 객체, 또는 null
 */
async function getConceptPhotoReply(userMessage, conversationContext) {
    ...
    // 키워드 -> 폴더 매핑 (긴 키워드 우선 매칭을 위해 길이 역순 정렬)
    ...
    // 이전에 보여준 컨셉이 있고 '다른 것도 보고싶어' 또는 '다음 사진' 요청 시
