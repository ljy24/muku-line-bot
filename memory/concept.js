// memory/concept.js v2.7 (concept-index.json의 컨셉명(폴더명)을 직접 키로 연동)
// [최종 수정] concept-index.json에서 폴더명 키를 사용하여 mood/episode 정보를 AI 멘트에 정확히 반영
...
async function loadConceptIndex() {
    try {
        ...
        conceptIndex = parsedData; // 전체 JSON 데이터를 conceptIndex에 저장

        // concept-index.json에서 'folders' 키를 찾아 폴더별 사진 개수 데이터 로드
...
async function getConceptPhotoReply(userMessage, conversationContext) {
    ...
    const conceptKeywords = [
        ...
        '청포도', '보라돌이', '밤바', '공원', '오타쿠', '힙', '캘빈', '네코', '후지 스냅', '이화마을' // 추가된 키워드
    ];
    const isConceptRequest = conceptKeywords.some(keyword => lowerCaseMessage.includes(keyword));
    
    // 키워드 -> 폴더명 매핑 (concept-index.json의 키 이름과 일치하도록)
    // 여기서의 키는 사용자 입력에 매칭될 키워드, 값은 CONCEPT_FOLDERS의 키 (실제 폴더명)
    const conceptKeywordMap = { 
        ...
        '피크닉': '2024_06_07_한국__피크닉', '벗꽃': '2024_04_12_한국_벗꽃', '후지_스냅': '2025_05_06_한국_후지_스냅', // '마지막_' 제거
        ...
        // concept-index.json에 있는 다른 긴 컨셉명들도 여기에 매핑 추가
        ...
        ,'2024_9월 15일 한국 옥상연리': '2024_9월 15일 한국 옥상연리'
        ,'2024_9월 16일 한국 길거리 스냅': '2024_9월 16일 한국 길거리 스냅'
        ,'2024_9월 17일 한국 을지로 스냅': '2024_9월 17일 한국 을지로 스냅'
        ,'2024_10월 16일 일본 결박': '2024_10_16_일본_결박'
        ,'2024_10월 16일 일본 욕실': '2024_10_16_일본_욕실'
        ...
        ,'2024_10_17일 일본 텐진 스트리트': '2024_10_17_일본_텐진_스트리트'
    ...
    if (lastConceptPhotoFolder && (lowerCaseMessage.includes('다른 것도 보고싶어') || lowerCaseMessage.includes('다음 사진'))) {
        ...
        // 개별 JPG 파일 (예: 2024_02_22_한국_생일_00022.jpg)은 '다음 사진' 개념이 없음
        // conceptIndex에서 해당 키가 'folders' 안에 있는지 확인하여 단일 파일이 아님을 확인
        if (!CONCEPT_FOLDERS[selectedFolder]) { // CONCEPT_FOLDERS에 없으면 단일 이미지일 가능성 높음
    ...
    let photoUrl;
    // selectedFolder가 직접 .jpg 파일명인 경우 (concept-index.json에 사진 개수가 1로 등록된 경우)
    // 또는 CONCEPT_FOLDERS에 폴더로 등록되지 않고 단일 이미지로만 존재하는 경우
    if (selectedFolder.endsWith('.jpg')) {
        photoUrl = generateConceptPhotoUrl(selectedFolder); // 이 경우 targetIndex는 무시됨
    ...
    // conceptIndex에서 해당 'selectedFolder' (컨셉명)의 mood, episode 정보 불러오기
    // 이제는 photoUrl에서 파일명을 추출하는 대신, 선택된 폴더명(컨셉명)을 직접 키로 사용
    const memory = conceptIndex[selectedFolder] || {};
    const memoryMood = memory.mood || '';
    const memoryEpisode = memory.episode || '';
    // 기존 comment 필드도 여전히 지원 (만약 mood/episode가 없는 경우 대비)
    const memoryComment = memory.comment || ''; 

    // GPT 시스템 프롬프트에 mood, episode 정보 추가 (우선순위: mood + episode > mood > episode > comment)
    let memoryPrompt = '';
    if (memoryMood && memoryEpisode) {
    ...
    } else if (memoryComment) { // 기존 comment 필드 지원
